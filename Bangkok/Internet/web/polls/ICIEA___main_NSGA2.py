...
if __name__=='__main__':
    ...
    def update_plot(i, data, pata, paretoX, paretoY,  paretoX_gen1, paretoY_gen1,  last, scat):
        ...
        if i>=(last-1):
            scat = plt.plot(paretoX,paretoY, c='b')
            scat = plt.scatter(data[NGEN-1],pata[NGEN-1], c='b')
            scat = plt.plot(paretoX_gen1,paretoY_gen1, c='r')
    ...
    ani = animation.FuncAnimation(fig1, update_plot, frames=xrange((NGEN-1)+5), fargs=( all_x,all_y, paretoX, paretoY, paretoX_gen1, paretoY_gen1,NGEN-1, scat),blit=False)
    ani.save('static/videos/bang' + str(888) + '.mp4', fps=3, writer="ffmpeg", extra_args=['-vcodec', 'libx264'])


    # ext = "mp4"
    # codec = {'mp4': 'libx264', 'webm': 'libvpx'}.get(ext, 'mpeg4')
    # ani.save("bang.mp4", fps=3, codec=codec, extra_args=['-vcodec', codec],
