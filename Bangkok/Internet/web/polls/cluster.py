...
class Cluster:
    #class variables
    description = "This is the Cluster class to use clustering methods"
    ...
    def EM(self, t, num_clusters):

        gmm = mixture.GMM(n_components=num_clusters, covariance_type='spherical')
        gmm.fit(t)

        return gmm


    #Expectation Maximization algorithm free Components K
    def EMfreeK(self, t):

        lowest_bic = np.infty
        bic = []
        n_components_range = range(1, self.k_max)
    ...
    #Kmeans
    def Kmeans(self, t, num_clusters):
    ...
    #Kmeans
    def KmeansfreeK(self, t):

        #before kmeans, discover number of components ... I used the daussian BIC process...
        lowest_bic = np.infty
        bic = []
        n_components_range = range(1, self.k_max)
        #cv_types = ['spherical', 'tied', 'diag', 'full']
        ...
        #kmeans
        k_means = KMeans(init='k-means++', n_clusters=best_gmm.n_components, n_init=10)
        kmm = k_means.fit(t)

        return kmm

    #Expectation Maximization algorithm
    def expectation_maximization(self, t, nbclusters=2, nbiter=1, normalize=False,epsilon=0.1, monotony=False, datasetinit=True):
        """
